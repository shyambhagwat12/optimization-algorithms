...
def loss_reformulate(A,b,x):
    return 0.5 * np.sum(np.square(np.dot(A, x) - b))


def gradient(x, A, b):
    return np.dot(np.dot(A.T, A), x) - np.dot(A.T, b) 

def franke_wolfe_step(gradient, l):
        a = np.argmax(np.abs(gradient))
        s = - np.sign(gradient[a]) * l
        return s

def frank_wolfe(update, A, b, reg, T=int(1e4)):
    x = np.zeros(A.shape[1])
    error = []
    l1 = []
    p = np.shape(A)[1]
